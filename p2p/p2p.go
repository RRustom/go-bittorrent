...
import (
  ...
  "github.com/RRustom/go-bittorrent/client"
...
func attemptDownloadPiece(c *client.Client, pw *pieceWork) ([]byte, error) {
  ...
  // set deadline to get unresponsive peers unstuck
  ...
  for state.downloaded < pw.length {
    ...
    if !state.client.Choked {
      for state.backlog < MaxBacklog && state.requested < pw.length {
        ...
        // last block might be shorter than the others
...
func (t *Torrent) startDownloadWorker(peer peers.Peer, workQueue chan *pieceWork, results chan *pieceResult) {
  ...
  if err != nil {
    log.Printf("Could not connect to %s. Disconnectiong\n", peer.IP)
    return
  }
  defer c.Conn.Close()
  ...
  for pw := range workQueue {
    if !c.Bitfield.HasPiece(pw.index) {
      workQueue <- pw // return piece to the queue
      return
    }
...
// downloads the torrent. Stores entire file in memory.
